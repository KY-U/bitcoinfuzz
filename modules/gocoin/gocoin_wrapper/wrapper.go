...
func GocoinSighashCompute(txData C.ByteArray, scriptData C.ByteArray, sigData C.ByteArray, inputIndex C.uint32_t, nCodesep C.uint32_t, amount C.uint64_t, sighashType C.uint32_t, isV0 C.int) *C.char {
	...
	// Both gocoin sighash functions always return a full sha256d digest;
	// anything else is a behavioral change worth crashing on, not skipping.
	if len(digest) != 32 {
		panic(fmt.Sprintf("gocoin sighash: unexpected digest length %d", len(digest)))
	}
