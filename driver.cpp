...
void Driver::Bech32RoundtripTarget(std::span<const uint8_t> buffer) const {
  ...
  // BIP-173 caps a segwit address at 90 characters, and the HRP at 83. How
  // much of the HRP range fits under the cap depends on the program: 78
  // characters for a 2 byte program, but only 18 for a 40 byte one. Drawing
  // the length uniformly from 1..83 would leave most inputs with a long
  // program over the cap, where they are executed but not compared (see
  // below). So the draw is biased: usually from the range that fits plus a
  // couple of characters past it, which is where the off-by-one bugs in the
  // length checks live, and occasionally from the full range so that
  // over-long HRPs still reach the encoders' output buffers.
  const size_t data_len{Bech32DataLength(program_len)};
  const size_t max_fitting_hrp_len{90 - 1 - data_len - 6};
  const size_t hrp_len{
      provider.ConsumeIntegralInRange<uint8_t>(0, 7) == 0
          ? provider.ConsumeIntegralInRange<size_t>(1, 83)
          : provider.ConsumeIntegralInRange<size_t>(
                1, std::min<size_t>(max_fitting_hrp_len + 2, 83))};
  ...
  // Over-cap inputs are still handed to every module -- an over-long HRP is
  // exactly what walks an encoder off the end of a fixed-size output buffer --
  // but their responses are not compared: implementations legitimately split
  // between refusing to encode and emitting a string their own decoder then
  // rejects, and that split would bury the checksum divergences this target is
  // looking for.
  const size_t encoded_len{input.hrp.size() + 1 + data_len + 6};
